...
def problems(env=None):
 ...
 missing=[k for k in REQUIRED if k not in CHAT_KEYS and not env.get(k,'').strip()]+list(chat.missing);invalid=list(chat.invalid)
 ...
 for k in ['EMBEDDING_BASE_URL','RERANKER_BASE_URL','MILVUS_URI']:
  if env.get(k):
   try:u=urlsplit(env[k]);local=u.hostname in {'127.0.0.1','localhost','::1'}
   except ValueError:invalid.append(k);continue
 ...
 for name,low,high in [('MODEL_TIMEOUT',5,120),('CHAT_MAX_TOKENS',32,8192)]:
  try:
   if env.get(name) and not low<=int(env[name])<=high:invalid.append(name)
 ...
 if env.get('JINSHU_MODEL_SCOPE')=='local' and chat.provider=='deepseek':invalid.append('JINSHU_MODEL_SCOPE')
...
def settings():
 ...
 return Settings(_env_file=None,storage_mode='mongo',vector_backend='milvus',app_name='金枢｜统一金融中后台工作台',seed_demo_users=False,
  ...
  relay_base_url=chat.base_url,relay_api_key=chat.api_key,relay_model=chat.model,deepseek_api_key='',
